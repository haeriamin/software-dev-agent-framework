...
3. Drive the SDD lifecycle: `/speckit.specify` → `/speckit.clarify` → `/speckit.plan` (Analyst fires via hook; Architect for HIGH/CRITICAL) → `/speckit.tasks` → `/speckit.implement`
...
Follow `.specify/extensions/dev/commands/dev.target.md`.

### review-escalated
Follow `.specify/extensions/dev/commands/dev.review-escalated.md` — present each escalation, wait for the human, record via Archivist, resume.
