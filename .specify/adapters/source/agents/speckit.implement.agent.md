...
Check `.specify/extensions.yml` for `hooks.before_implement` / `hooks.after_implement` per [extension hook protocol](../instructions/extension-hooks.instructions.md). In this framework `after_implement` → `/dev.review` is **mandatory** — the slice is not complete without a Reviewer verdict.

## Outline

1. Verify prerequisites: `check-prerequisites -Phase implement` (spec + plan + tasks; design Approved for HIGH/CRITICAL).
2. **Bootstrap** (Principle II — full sequence). Verify reversibility setup (Principle VI): git target → branch `sdd/<slice>` created from the default branch; non-git → `work-queue/backups/<slice>/` staged.
3. Move/create the work item in `work-queue/in-progress/` (status: IN_PROGRESS).
...
6. **Review** (mandatory after_implement hook) `/dev.review`:
   - PASS → work item → `completed/`; report flagged items: none.
   - CONDITIONAL_PASS → `completed/` with the flagged-items list; request human spot-check.
...
8. Report: verdict, confidence with sub-scores, files changed, branch name + rollback procedure, PARTIAL/escalation count, log entries appended. **Never merge or push** — present the branch for human merge.
