...
Check `.specify/extensions.yml` for `hooks.before_analyze` / `hooks.after_analyze` per [extension hook protocol](../instructions/extension-hooks.instructions.md) (`after_analyze` offers `/dev.audit`).

## Outline

1. Load all artifacts of the active slice: `spec.md`, `plan.md`, `design.md` (if any), `tasks.md`, implementation report, test report, review report.
2. **Consistency matrix** — check and report each:
   ...
   - Citations resolve (spot-check N=5 Decision Records against /standards/ sources).
...
5. Report the matrix with verdicts per row. Append to `wiki/log.md`.
