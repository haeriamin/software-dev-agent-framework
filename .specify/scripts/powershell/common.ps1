# common.ps1 — shared helpers for speckit core scripts (dot-source this file).

function Get-RepoRoot {
    $dir = Get-Location
    while ($dir) {
        if (Test-Path (Join-Path $dir ".specify\memory\constitution.md")) { return $dir.ToString() }
        $parent = Split-Path $dir -Parent
        if ($parent -eq $dir.ToString()) { break }
        $dir = $parent
    }
    throw "Not inside the framework (no .specify/memory/constitution.md found upward from $(Get-Location))."
}

function Get-FeatureDirectory {
    # Resolution order: .specify/feature.json → highest-numbered specs/NNN-* dir.
    $root = Get-RepoRoot
    ...
    if ($null -eq $latest) { throw "No feature directory found. Run /speckit.specify first." }
