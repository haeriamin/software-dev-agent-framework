...
`.specify/adapters/source/`; edit the source and regenerate, do not hand-edit these files.

## What's here
| Piece | Location | Notes |
|---|---|---|
| 8 personas | `.codex/agents/*.toml` | `developer_instructions` = shared persona body; `sandbox_mode` from source |
| 22 commands | `.codex/prompts/*.md` | `/dev.*` + `/speckit.*`; install into `~/.codex/prompts/` |
...
`/speckit.specify`, etc. (dot syntax).
