...
$templatePath = Join-Path $root ".specify\templates\$Template"
if (-not (Test-Path $templatePath)) { throw "Template not found: $templatePath" }
$specFile = Join-Path $featureDir "spec.md"
Copy-Item $templatePath $specFile

$relDir = "specs/$num-$ShortName"
@{ feature_directory = $relDir } | ConvertTo-Json | Set-Content (Join-Path $root ".specify\feature.json") -Encoding utf8
