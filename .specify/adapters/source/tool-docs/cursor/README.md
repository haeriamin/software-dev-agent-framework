...
(constitution, runbooks, speckit agents, instructions, skills). Do not hand-edit these files — they
are rendered by `tools/convert` from `.specify/adapters/source/`. Edit the source and regenerate.

## What loads from here

| Cursor feature | Files | Maps to |
|----------------|-------|---------|
| Subagents | `.cursor/agents/*.md` | the eight personas (`.specify/adapters/source/personas/`) |
| Slash commands | `.cursor/commands/*.md` | `/dev.*` and `/speckit.*`, each pointing at its runbook |
...
Slash syntax is the dot form: `/dev.analyze`, `/speckit.specify`.
...
`.specify/adapters/generated/cursor/hooks.template.json`. `tools/setup-hooks` installs the real
