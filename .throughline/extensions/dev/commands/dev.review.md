...
**Agent**: Reviewer
**Reads**: `specs/NNN-*/{spec,plan,design,tasks}.md`, implementation report, test report, target diff (branch vs base), `/standards/**` DIRECTLY (not wiki summaries), `/exemplars/**` for cited bases
**Writes**: `review-reports/<target>/<slice>-review.md`; append to `wiki/log.md`
**Never writes**: target source, `/standards/**`, `/exemplars/**`, `/wiki/**` (except log)

## Preconditions

- Implementation report exists.
- **Fresh test report exists** (`review-reports/<target>/<slice>-tests.md` with a source
  hash matching the current branch state). Missing/stale → invoke `/dev.test` first;
  if it cannot run, proceed with `test_evidence = 0`.

## Steps

1. **Bootstrap** (Principle II — full sequence).
2. **Independence rule**: re-read every cited standard clause from `/standards/` source
   files and every cited exemplar from `/exemplars/` — never trust the wiki summary or
...
3. **Layer 1 — Structural**: lint state, naming conventions, file organization,
   ...
   `DEV-STATUS: PARTIAL` annotation has a matching exception-registry/escalation entry.
...
8. Write `review-reports/<target>/<slice>-review.md`: verdict, the three sub-scores with
   their numerators/denominators, itemized findings (each with file:line + standard
...
10. Routing: FAIL → back to `/dev.implement` with the findings (max 2 retries, then
    `/dev.review-escalated`). PASS/CONDITIONAL_PASS → Orchestrator moves the work item
    to `work-queue/completed/`; merging/pushing remains a human action (Principle VI).
