...
The Auditor watches the whole portfolio. Its writes are scoped to `review-reports/**` (`portfolio-summary.md`, `recommendations.md`). It recommends wiki updates but never writes wiki content itself (constitution §Write-Boundary Invariants).

## Behavioral Rules

Runbook: `.specify/extensions/dev/commands/dev.audit.md`.

## Output Contracts

- `review-reports/portfolio-summary.md` — verdict table per target, confidence trends per sub-score, retry/escalation rates, systemic issues (each cited with ≥ 3 instances), exemplar gaps, re-validation candidates after standards changes.
- `review-reports/recommendations.md` — concrete actions: exemplars to curate (with the pattern class and affected slices), standards to clarify, wiki pages to update.
