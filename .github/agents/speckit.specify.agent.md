...
Check `.specify/extensions.yml` for `hooks.before_specify` / `hooks.after_specify` per [extension hook protocol](../instructions/extension-hooks.instructions.md).

## Outline

The text after `/speckit.specify` **is** the slice description. Do not ask the user to repeat it.
...
3. **Create the feature directory** by running the helper script:
   - PowerShell: `.specify/scripts/powershell/create-new-feature.ps1 -ShortName "<short-name>"`
   - Bash: `.specify/scripts/bash/create-new-feature.sh <short-name>`
   - The script copies `.specify/templates/spec-template.md` to `specs/NNN-<short-name>/spec.md`, creates `checklists/`, and persists the path to `.specify/feature.json`. One feature per invocation.
...
5. **Validate** against `specs/NNN-*/checklists/requirements.md` (create from `.specify/templates/checklist-template.md`):
...
6. **Report**: FEATURE_DIR, SPEC_FILE, target id, checklist results, readiness for `/speckit.clarify` or `/speckit.plan`. Append to `wiki/log.md`.
