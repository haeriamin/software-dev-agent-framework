...
writes an ideation note you can carry into `/dev.specify`, but it never writes a spec, a branch,
...
7. **Write the ideation note** (`work-queue/pending/<topic>-ideation.md`):
   ...
   **Target**: <id or "none yet"> | **Date**: ... | **Status**: exploring
   ...
   - Suggested next step: /dev.specify "<recommended direction>" (target: <id>)
   ```
...
`/dev.specify "<direction>" (target: <id>)` (or `/dev.feature`), which will re-derive a proper
