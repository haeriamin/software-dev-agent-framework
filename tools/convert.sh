...
# of truth in .specify/adapters/source/, driven by the per-tool profiles in
# .specify/adapters/profiles/. Output is byte-identical to convert.ps1 (LF line endings, same
...
SRC="$REPO_ROOT/.specify/adapters/source"
PROFILES="$REPO_ROOT/.specify/adapters/profiles"
...
get_field() {
  ...
  val="$(awk -v k="$key" 'index($0, k" = ")==1 { print substr($0, length(k)+4); exit }' "$file")"
  # Trim leading/trailing whitespace (parity with convert.ps1 ConvertFrom-MetaText).
...
apply_slash() {
  if [ "$1" = ":" ]; then cat; else sed -e 's#/dev:#/dev'"$1"'#g' -e 's#/speckit:#/speckit'"$1"'#g'; fi
}
...
build_command_core() {
  ...
  if [ "$ns" = "dev" ]; then
    ...
    runbook=".specify/extensions/dev/commands/dev.$cmd.md"
    local tailpart=""; [ -n "$tail" ] && tailpart=" $tail"
    printf '%s\n\n%s\n\n%s' \
      "Adopt the **$persona** ${word}persona (\`$ref\`) $DASH $action" \
      "Follow the canonical runbook at \`$runbook\` step-by-step: ${steps}.${tailpart} The constitution at \`.specify/memory/constitution.md\` overrides everything else." \
      "$final"
  else
    runbook=".github/agents/speckit.$cmd.agent.md"
    printf '%s\n\n%s' \
      "Follow the runbook at \`$runbook\` $DASH its content is runtime-neutral; ignore Copilot-specific frontmatter (tools/handoffs) and use your own tools. Helper scripts referenced there live under \`.specify/scripts/\` (PowerShell and bash variants)." \
      "Check \`.specify/extensions.yml\` for lifecycle hooks per \`.github/instructions/extension-hooks.instructions.md\`. The constitution at \`.specify/memory/constitution.md\` overrides everything else."
  fi
}
...
emit_personas() {
  ...
  for p in "$SRC"/personas/*.persona; do
    ...
    case "$fmt" in
      md)        printf -- '---\nname: %s\ndescription: %s\ntools: %s\n---\n\n%s\n\n<!-- generated by tools/convert from .specify/adapters/source/agents/%s.agent.md + personas/%s.persona; edit the source, not this file. -->' "$name" "$desc" "$tools" "$body" "$name" "$name" | apply_slash "$sep" | write_generated "$dir/$name$ext" ;;
      md-cursor) printf -- '---\nname: %s\ndescription: %s\nmodel: inherit\nreadonly: %s\n---\n\n%s\n\n<!-- generated by tools/convert from .specify/adapters/source/personas/%s.persona; edit there. -->' "$name" "$desc" "$ro" "$body" "$name" | apply_slash "$sep" | write_generated "$dir/$name$ext" ;;
      md-antigravity) printf -- '---\nname: %s\ndescription: %s\n---\n\n%s\n\n<!-- generated by tools/convert from .specify/adapters/source/personas/%s.persona; edit there. -->' "$name" "$desc" "$body" "$name" | apply_slash "$sep" | write_generated "$dir/$name$ext" ;;
      md-opencode)
        if [ "$ro" = "true" ]; then
          printf -- '---\ndescription: %s\nmode: subagent\npermission:\n  edit: deny\n  bash:\n    "*": ask\n---\n\n%s\n\n<!-- generated by tools/convert from .specify/adapters/source/personas/%s.persona; edit there. -->' "$desc" "$body" "$name" | apply_slash "$sep" | write_generated "$dir/$name$ext"
        else
          printf -- '---\ndescription: %s\nmode: subagent\n---\n\n%s\n\n<!-- generated by tools/convert from .specify/adapters/source/personas/%s.persona; edit there. -->' "$desc" "$body" "$name" | apply_slash "$sep" | write_generated "$dir/$name$ext"
        fi ;;
      md-qwen) printf -- '---\nname: %s\ndescription: %s\n---\n\n%s\n\n<!-- generated by tools/convert from .specify/adapters/source/personas/%s.persona; edit there. -->' "$name" "$desc" "$body" "$name" | apply_slash "$sep" | write_generated "$dir/$name$ext" ;;
      md-kimi) printf -- '<!-- generated by tools/convert from .specify/adapters/source/personas/%s.persona; edit there. -->\n\n# %s\n\n%s' "$name" "$disp" "$body" | apply_slash "$sep" | write_generated "$dir/$name$ext" ;;
      toml)      sandbox_line="sandbox_mode = \"$sandbox\""
                 [ -n "$sbcomment" ] && sandbox_line="$sandbox_line   # $sbcomment"
                 instr="$body"
                 [ -n "$note" ] && instr="$body"$'\n\n'"$note"
                 printf -- '# GENERATED by tools/convert from .specify/adapters/source/personas/%s.persona %s edit there.\n# Peer to .claude/agents/%s.md and .github/agents/%s.agent.md (shared, runtime-neutral).\n\nname = "%s"\ndescription = "%s"\nmodel_reasoning_effort = "high"\n%s\n\ndeveloper_instructions = """\n%s\n"""\n' "$name" "$DASH" "$name" "$name" "$disp" "$(esc_yaml "$desc")" "$sandbox_line" "$instr" | apply_slash "$sep" | write_generated "$dir/$name$ext" ;;
...
emit_commands() {
  ...
  for c in "$SRC"/commands/*.command; do
    ...
    case "$fmt" in
      ...
      codex)  lead="<!-- Codex CLI adapter for /$full $DASH generated from .specify/adapters/source. Filename ($full.md) is the slash command; install by copying .codex/prompts/*.md into \$CODEX_HOME/prompts/. -->"
              build_frontmatter_command "$c" codex "$pf" "$lead" | apply_slash "$sep" | write_generated "$path" ;;
      cursor) core="$(build_command_core "$c" cursor "$pf")"
              printf -- '<!-- generated by tools/convert from .specify/adapters/source/commands/%s.command; edit there. -->\n\n# /%s\n\n%s\n\nUser input follows after the command: $ARGUMENTS' "$full" "$full" "$core" | apply_slash "$sep" | write_generated "$path" ;;
...
emit_hooks() {
  local pf="$1" fmt path base="github/hooks/scripts"
  ...
  if [ "$fmt" = "github-json" ]; then
    {
      printf '%s\n' "{"
      printf '%s\n' '  "_generated": "by tools/convert from .specify/adapters/source/hook-spec.tsv; cross-OS (the windows field carries the PowerShell variant)",'
      printf '%s\n' '  "hooks": {'
  ...
  elif [ "$fmt" = "cursor-json" ]; then
    {
      printf '%s\n' "{"
      printf '%s\n' '  "version": 1,'
      printf '%s\n' '  "_generated": "by tools/convert from .specify/adapters/source/hook-spec.tsv; tools/setup-hooks installs this to .cursor/hooks.json and rewrites commands per OS. failClosed is false until the verification spike passes.",'
      printf '%s\n' '  "hooks": {'
      local i
      printf '    "preToolUse": [\n'
    ...
    {
      printf '%s\n' "{"
      printf '%s\n' '  "_generated": "by tools/convert from .specify/adapters/source/hook-spec.tsv; tools/setup-hooks installs this to .agents/hooks.json and rewrites commands per OS. Matchers are best-effort until the verification spike passes.",'
      printf '%s\n' '  "hooks": {'
      printf '%s\n' '    "PreToolUse": ['
...
emit_rules() {
  ...
  if [ "$fmt" = "mdc" ]; then
    ...
    disp="$(get_field "$pf" display "$(get_field "$pf" id)")"
    {
      printf '%s\n\n' "<!-- generated by tools/convert from .specify/adapters/source/global-rules.md + profiles/antigravity.profile; edit the source, not this file. -->"
      ...
      printf -- '- **Canonical brain**: `.specify/extensions/dev/commands/`, `.github/instructions/`, `/standards/`.\n\n'
      printf '> Preview adapter %s see `.agents/VERIFICATION.md` before trusting hook enforcement.\n' "$DASH"
    } | apply_slash "$sep" | write_generated "$path"
    rex="$(get_field "$pf" rules_extra_file)"
    if [ -n "$rex" ]; then
      printf '%s\n\n%s' "<!-- generated by tools/convert from .specify/adapters/source/global-rules.md; edit there. -->" "$global" \
        | apply_slash "$sep" | write_generated "$REPO_ROOT/$rex"
    fi
  elif [ "$fmt" = "qwen" ]; then
    {
      printf '%s\n\n' "<!-- generated by tools/convert from .specify/adapters/source/global-rules.md + profiles/qwen.profile; edit the source, not this file. -->"
    ...
    {
      printf '%s\n\n' "<!-- generated by tools/convert from .specify/adapters/source/global-rules.md + profiles/kimi.profile; edit the source, not this file. -->"
    ...
    {
      printf '%s\n\n' "<!-- generated by tools/convert from .specify/adapters/source/global-rules.md + profiles/opencode.profile; edit the source, not this file. -->"
...
emit_manifest() {
  ...
  is_true "$pf" emit_config     && entries+=("    \"config_file\": \"$(get_field "$pf" config_file)\"")
  {
    ...
    printf '  "generated_by": "tools/convert from .specify/adapters/source + profiles/%s.profile",\n' "$id"
    printf '  "entry_points": {\n'
    local n=${#entries[@]} k
    for ((k=0;k<n;k++)); do if [ $k -lt $((n-1)) ]; then printf '%s,\n' "${entries[$k]}"; else printf '%s\n' "${entries[$k]}"; fi; done
    printf '  },\n'
    printf '  "notes": "Thin adapter generated from the single source. Canonical procedure lives in .specify/extensions/dev/commands/ and .specify/adapters/source/agents/."\n'
    printf '}'
  } | write_generated "$path"
}
...
copy_source_tree() {
  ...
  while IFS= read -r -d '' f; do
    ...
    if [ "$marker" -eq 1 ] && [[ "$f" == *.md ]] && ! head -n1 "$f" | grep -q 'generated by tools/convert'; then
      { printf '%s\n\n' "<!-- generated by tools/convert from .specify/adapters/source/$srcSub/$rel; edit the source, not this file. -->"
        cat "$f"
      } | write_generated "$out"
    else
...
emit_tool_docs() {
  ...
  emit_one() {
    ...
    if [[ "$rel" == *.md ]] && ! head -n1 "$src" | grep -q 'generated by tools/convert'; then
      { printf '%s\n\n' "<!-- generated by tools/convert from .specify/adapters/source/tool-docs/$rel; edit the source, not this file. -->"
        cat "$src"
...
emit_shared() {
  echo "[shared] canonical GitHub + globals from .specify/adapters/source/"
  ...
  for name in AGENTS.md CLAUDE.md copilot-instructions.md claude-settings.json pull_request_template.md; do
    [ -f "$gdir/$name" ] || continue
    case "$name" in
      AGENTS.md) dest="AGENTS.md" ;;
      CLAUDE.md) dest="CLAUDE.md" ;;
    ...
    if [[ "$name" == *.md ]] && ! head -n1 "$gdir/$name" | grep -q 'generated by tools/convert'; then
      { printf '%s\n\n' "<!-- generated by tools/convert from .specify/adapters/source/globals/$name; edit the source, not this file. -->"
        cat "$gdir/$name"
      } | write_generated "$REPO_ROOT/$dest"
    else
