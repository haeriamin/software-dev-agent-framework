...
> Shared by all speckit.* lifecycle agents. Defines how to check and surface extension hooks.

Every speckit lifecycle command checks `.specify/extensions.yml` at two points: **before** starting its phase (`hooks.before_<phase>`) and **after** reporting completion (`hooks.after_<phase>`).

## Procedure

1. Check if `.specify/extensions.yml` exists in the project root. Missing → skip silently.
